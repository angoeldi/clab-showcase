...
def build_graph(*, domain_path: str | None = None, tutorial_path: str | None = None, checkpointer: object | None = None):
    domain_path = domain_path or (
        _env("CLAB_DOMAIN_PATH", "configs/examples/mi-social-media-addiction/domain.yaml")
        or "configs/examples/mi-social-media-addiction/domain.yaml"
    )
    tutorial_path = tutorial_path or (
        _env("CLAB_TUTORIAL_PATH", "configs/examples/mi-social-media-addiction/tutorial.yaml")
        or "configs/examples/mi-social-media-addiction/tutorial.yaml"
    )
    ...
    async def analyze(state: GraphState) -> GraphState:
        state = _init_state(state)
        state["generated_reasoning"] = []
        _emit_reasoning_status("analyze", "Analyzing your message and change context.", "start")
    ...
    async def act(state: GraphState) -> GraphState:
        state = _init_state(state)
        state["generated_reasoning"] = []
        _emit_reasoning_status("act", "Selecting the next MI intervention.", "start")
    ...
    async def compose(state: GraphState) -> GraphState:
        state = _init_state(state)
        state["generated_reasoning"] = []
        _emit_reasoning_status("compose", "Composing a supportive response.", "start")
        analysis = AnalysisResult.model_validate(state.get("analysis") or {})
        ...
        instructions = (
            ...
            "- The user-facing reply should read like a direct coaching response, not a diagnostic report.\n"
