...
DEFAULT_DOMAIN_PATH = "configs/examples/mi-social-media-addiction/domain.yaml"
DEFAULT_TUTORIAL_PATH = "configs/examples/mi-social-media-addiction/tutorial.yaml"
DEFAULT_USERS_PATH = "configs/examples/mi-social-media-addiction/users.yaml"
DEFAULT_USERS_STATE_PATH = "./.clab/user_sessions.json"
DEFAULT_REGISTERED_USERS_STATE_PATH = "./.clab/registered_users.json"
DEFAULT_UI_TITLE = "MI Social Media Coach"
DEFAULT_ASSISTANT_NAME = "MI Coach"
DEFAULT_AUTH_REALM = "mi-social-media-coach"
...
app = FastAPI(title="mi-social-media-addiction-example", version="0.1.0", lifespan=lifespan)
...
def _node_deterministic_reasoning(node: str, data: Dict[str, Any]) -> list[str]:
    ...
    if node == "analyze":
        ...
        return [f"I analyzed your message as stage **{stage}** and estimated risk at {risk}/5 for this turn."]
    ...
    if node == "compose":
        ...
        return [f"I composed your response according to the **{action}** plan and current context."]

    if node == "finalize":
        return ["I finalized the turn by storing the response and updated status for the next message."]
...
def _node_status_message(node: str, node_update: Any) -> str | None:
    ...
    if node == "analyze":
        ...
        if stage:
            return f"Message analysis complete ({stage})."
    ...
    if node == "act":
        ...
        if action:
            return f"Intervention selected ({action})."
